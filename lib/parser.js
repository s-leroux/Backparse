...
class Parser {
  constructor(startRule) {
    ...
    this._astHead = { // XXX Is this usedful? Why not null?
      trace: "ast-head",
      arity: 0,
      next: null,
    };
  ...
  */
  accept(tk) {
    ...
    while(this._token && this._state) {
      ...
      for(let astNode = this._astHead; astNode; astNode = astNode.next) {
        process.stdout.write(astNode.trace);
        process.stdout.write(":");
        process.stdout.write(astNode.token?.value || "");
        process.stdout.write(" -> ");
      }
      process.stdout.write("\n");
    }
...
function rule(name, n=0) {
  function _rule(parser) {
    ...
    if (form === undefined) {
      ...
      parser._astHead = parser._state.astHead;
