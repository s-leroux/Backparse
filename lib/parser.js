...
class State {
  constructor(failPath, successPath, opt) {
    ...
    this.savedToken = opt.savedToken;
    if (this.savedToken === undefined)
      this.savedToken = successPath?.savedToken ?? null;

    // only 'undefined' is a special case. 'null' is not
    // so we can't use the ?? operator here.
    this.astHead = opt.astHead;
    if (this.astHead === undefined)
      this.astHead = successPath?.astHead ?? null;
  }

  clone(trace, matcher) {
    return new State(this.failPath, this.successPath, {
      trace: trace,
      matcher: matcher,
      value: this.value,
...
function several(next) {
  function __several(parser) {
    ...
    const newSuccess = new State(this.successPath, this.successPath, {
      trace: "again?",
      matcher: __several,
    });

    return new State(this.successPath, newSuccess, {
      trace: "iter",
      matcher: next,
      // save the parser's state to restore in case of failure
...
function rule(name, n=0) {
  function _rule(parser) {
    ...
    if (form === undefined) {
      ...
      let failPath = this.clone(name + " F " + (n+1), rule(name, n+1) );

      let lastSuccessPath = new State(failPath, savedSuccessPath, {
        trace: name + " S " + (n+1),
        // BTW, no need to save the parser's state
        // since we can't return here from a failure.
        matcher(parser) {
      ...
      while(i--) {
        lastSuccessPath = new State(failPath, lastSuccessPath, {
          ...
          value: name,
          matcher: form[i],
          // if anything fails, retore the parser's state:
          savedToken: parser._token,
          astHead: parser._astHead,
        });
      }
